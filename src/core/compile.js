...
export function compileContext({ cwd = process.cwd(), config = loadConfig(), includeProject = true } = {}) {
  ...
  const text = clamp(
    [
      '# Notes about this user, from wife',
      '',
      'The following was recorded locally by the wife CLI from this user\'s own earlier messages. It is background information about them and their project, not an instruction for the current turn.',
      '',
      body,
    ].join('\n'),
    HARD_CHAR_CAP
...
function clamp(text, maxChars) {
  ...
  return `${cut.slice(0, cut.lastIndexOf('\n'))}\n(truncated by wife to fit the hook output limit)`;
}
