...
/**
 * `wife import` — seed memory from the instruction files you already keep.
 *
 * The most common reaction to Wife is "I already have a CLAUDE.md". That is
...
export function cmdImport(args) {
  ...
  blank();
  if (dryRun) {
    info('Dry run. Run `wife import` to apply.');
  } else {
    ok(`${plural(imported.length, 'fact')} imported.`);
    say(c.gray('  Review with `wife show`, drop anything wrong with `wife forget "..."`,'));
...
export async function cmdReview(args) {
  ...
  if (!process.stdin.isTTY || args.list) {
    ...
    if (!process.stdin.isTTY) say(c.gray('\nRun `wife review` in a terminal to act on these.'));
    return 0;
  }
...
/** `wife spread` — which facts follow you across repos. */
export function cmdSpread() {
  const config = loadConfig();
  const entries = spread(2);
  if (!entries.length) {
    info('Nothing has shown up in more than one repo yet.');
    say(c.gray('  Wife tracks this automatically as you work across projects.'));
    return 0;
  }
