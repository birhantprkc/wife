...
/**
 * `wife harden` — offer to enforce the rules that can be enforced.
 *
...
export async function cmdHarden(args) {
  ...
  if (!found.length) {
    ...
    if (active.length) {
      ...
      say(c.gray('  See them with `wife guards`.'));
    } else {
      info('No rules found that a hook could enforce.');
      say(c.gray('  Most of what wife remembers — tone, language, preferences —'));
  ...
  if (!process.stdin.isTTY || args.list) {
    ...
    say(c.gray('\n  Run `wife harden` in a terminal to turn these on.'));
    return 0;
  }
...
/** `wife guards` — what is actually being enforced, and `--off <text>` to stop. */
export function cmdGuards(args) {
  ...
  if (args.add) {
    ...
    if (!rule || !blocks) {
      fail('Usage: wife guards --add "<your rule>" --blocks "<command fragment>"');
      say(c.gray('  e.g.  wife guards --add "Nunca terraform apply en prod" --blocks "terraform apply"'));
      say(c.gray('        wife guards --add "No toques la config" --blocks "config/prod" --tool Edit'));
  ...
  if (args.test) {
    ...
    if (!command) { fail('Usage: wife guards --test "<command>"'); return 1; }
  ...
  if (!state.guards.length) {
    ...
    say(c.gray('  Run `wife harden` to see which of your rules could be.'));
    return 0;
  }
  ...
  say(c.gray('  Everything else wife knows stays as context: it has no tool call to intercept.'));
  say(c.gray('  Add one the detectors miss: wife guards --add "<rule>" --blocks "<command>"'));
  return 0;
}
