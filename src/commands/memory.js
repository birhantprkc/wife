...
/** `wife remember "<fact>"` — the manual override. Always trusted, never staged. */
export function cmdRemember(args) {
  const text = args._.join(' ').trim();
  if (!text) {
    fail('Nothing to remember. Try: wife remember "I prefer short answers"');
    return 1;
  }
  ...
  if (pruned.length) warn(`Budget reached; dropped ${pruned.length} lower-value fact(s). See \`wife journal\`.`);
  return 0;
}

/** `wife forget "<text or id>"` — removes the line entirely. No softening. */
export function cmdForget(args) {
  const query = args._.join(' ').trim();
  if (!query) {
    fail('Nothing to forget. Try: wife forget "short answers"');
    return 1;
  }
  const config = loadConfig();
  const targets = [
  ...
  for (const m of matches) {
    m.store.remove(m.fact.id, 'user ran `wife forget`');
    m.store.save();
...
/** `wife pin "<text>"` — exempt a fact from decay and from budget eviction. */
export function cmdPin(args, { unpin = false } = {}) {
  const query = args._.join(' ').trim();
  if (!query) {
    fail(`Usage: wife ${unpin ? 'unpin' : 'pin'} "<text>"`);
    return 1;
  }
  const config = loadConfig();
...
/**
 * `wife why "<text>"` — the trust command.
 *
...
export function cmdWhy(args) {
  const query = args._.join(' ').trim();
  if (!query) {
    fail('Usage: wife why "<fact or fragment>"');
    return 1;
  }
  const config = loadConfig();
  const found = [];
...
function describeSource(source) {
  return {
    manual: 'you typed it into `wife remember`',
