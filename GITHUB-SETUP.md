...
Website field: `https://github.com/ma-nucho-pro/wife#readme`
...
**Title:** `v1.0.0 — Wife remembers everything`
...
Wife gives Claude Code and Codex a memory that survives the session boundary —
...
- Tells you where every fact came from: `wife why "<text>"`.
...
**Human scanning.** The logo, tagline and the `wife show` output appear before any prose. Someone deciding in four seconds sees the result first, not an explanation of it.
...
**Day 2–3 — communities.** r/ClaudeAI, r/ChatGPTCoding, r/LocalLLaMA, the Anthropic Discord, dev.to. Post the problem first and the tool second. "I got tired of re-explaining my stack every morning, so I built this" outperforms "Introducing Wife v1.0" by a wide margin.

**Day 4+ — aggregators.** Hacker News (Show HN), Product Hunt, Lobsters. Show HN wants the story of why you built it. Title: `Show HN: Wife – local memory layer for Claude Code and Codex`.
...
**Pin one issue** titled `Which phrasings should Wife learn?` and invite people to paste the sentences it missed. That turns your weakest point into a contribution funnel, and each reply is free extractor training data.

**Add a demo GIF to the README** once you have one. Record `wife show`, then a fresh session where the agent already knows you. A GIF above the fold measurably outperforms a code block.
...
```markdown
<img src="https://img.shields.io/github/stars/ma-nucho-pro/wife?style=for-the-badge&color=f2c94c" alt="stars" />
```
