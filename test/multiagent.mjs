...
const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'wife-agents-e2e-'));
...
const env = {
  ...process.env,
  WIFE_HOME: path.join(sandbox, '.wife'),
...
wife(['init']);
wife(['remember', 'Prefiere respuestas cortas']);
wife(['remember', '--project', 'Nunca hagas force push']);

// ---------------------------------------------------------------------------
section('claude code');
let r = wife(['attach', 'claude']);
...
r = wife(['inject'], JSON.stringify({ session_id: 'c1', cwd: repo, hook_event_name: 'SessionStart', source: 'startup' }));
check('inject returns the memory', r.status === 0 && /respuestas cortas/i.test(r.stdout), r.stdout.slice(0, 120));

r = wife(['capture'], JSON.stringify({ session_id: 'c1', cwd: repo, prompt: 'recuerda que uso Kotlin a diario' }));
check('capture is silent and exits 0', r.status === 0 && r.stdout.trim() === '', JSON.stringify(r.stdout));
r = wife(['harvest', '--stdin'], JSON.stringify({ session_id: 'c1', hook_event_name: 'SessionEnd' }));
check('harvest exits 0', r.status === 0, r.stderr);
check('LEARNED from a Claude Code session',
  /Kotlin/i.test(fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8')));
...
r = wife(['inject', '--agent', 'codex'], codexPayload({ source: 'startup' }));
...
r = wife(['capture', '--agent', 'codex'], codexPayload({ hook_event_name: 'UserPromptSubmit', turn_id: 't1', prompt: 'recuerda que despliego los martes' }));
check('capture accepts the Codex payload', r.status === 0, r.stderr);
r = wife(['harvest', '--stdin', '--quiet'], codexPayload({ hook_event_name: 'Stop', turn_id: 't1' }));
check('harvest runs on Stop', r.status === 0, r.stderr);
check('LEARNED from a Codex session',
  /martes/i.test(fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8')),
  fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8'));

// ---------------------------------------------------------------------------
section('cursor — different field names, different block format');
r = wife(['attach', 'cursor']);
check('attach exits 0', r.status === 0, r.stderr);
...
check('writes an always-applied rules file',
  fs.readFileSync(path.join(repo, '.cursor', 'rules', 'wife-memory.mdc'), 'utf8').includes('alwaysApply: true'));

// Cursor sends conversation_id and text, not session_id and prompt
r = wife(['capture', '--agent', 'cursor'], JSON.stringify({
...
r = wife(['harvest', '--stdin', '--quiet', '--refresh-cursor'], JSON.stringify({
  conversation_id: 'cur-1', hook_event_name: 'stop', workspace_roots: [repo],
}));
check('harvest runs on stop', r.status === 0, r.stderr);
check('LEARNED from a Cursor session',
  /PR|tarde/i.test(fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8')),
  fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8'));
check('and the rules file was refreshed with it',
  /PR|tarde/i.test(fs.readFileSync(path.join(repo, '.cursor', 'rules', 'wife-memory.mdc'), 'utf8')));

// ---------------------------------------------------------------------------
section('gemini cli — injection only, and it says so');
r = wife(['attach', 'gemini']);
check('attach exits 0', r.status === 0, r.stderr);
...
r = wife(['guard', '--style', 'codex'], JSON.stringify({ tool_name: 'Bash', tool_input: { command: 'git push --force' }, cwd: repo }));
...
r = wife(['guard', '--style', 'codex'], JSON.stringify({ tool_name: 'Bash', tool_input: { command: 'ls' }, cwd: repo }));
check('codex: allows with exit code 0', r.status === 0, `exit ${r.status}`);

r = wife(['guard', '--style', 'cursor'], JSON.stringify({
...
check('no wife hooks left in Claude Code',
  !JSON.stringify(JSON.parse(fs.readFileSync(path.join(sandbox, '.claude', 'settings.json'), 'utf8'))).includes('wife.js'));
check('no wife hooks left in Codex',
  !JSON.stringify(JSON.parse(fs.readFileSync(codexHooksFile, 'utf8'))).includes('wife.js'));
check('no wife hooks left in Cursor',
  !JSON.stringify(JSON.parse(fs.readFileSync(cursorHooksFile, 'utf8'))).includes('wife.js'));
check('the Cursor rules file is gone too',
  !fs.existsSync(path.join(repo, '.cursor', 'rules', 'wife-memory.mdc')));
check('memory itself survived every detach',
  fs.readFileSync(path.join(sandbox, '.wife', 'identity.md'), 'utf8').includes('respuestas cortas'));
