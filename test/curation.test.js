...
beforeEach(() => {
  sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'wife-curation-'));
...
describe('import — from the files people already keep', () => {
  ...
  test("Wife never re-imports its own managed block", () => {
    const doc = `# Mine\n\n## Style\n- Use tabs\n\n${BEGIN}\n\n## About the person\n- Prefers short answers\n\n${END}\n`;
    assert.ok(!stripManaged(doc).includes('Prefers short answers'), 'feedback loop: Wife would re-learn its own output');
